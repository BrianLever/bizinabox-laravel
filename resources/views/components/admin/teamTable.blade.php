            ...
            <tr>
                ...
                <td>
                    @foreach($team->users->where('pivot.role', 'client') as $client)
                        <img src="{{$client->avatar()}}" alt="{{$client->name}}" class="tooltip_1 rounded-circle width-50px" title="{{$client->name}} (Client)">
                    @endforeach
                    @foreach($team->users->where('pivot.role', 'employee') as $employee)
                        <img src="{{$employee->avatar()}}" alt="{{$employee->name}}" class="tooltip_1 rounded-circle width-50px" title="{{$employee->name}} (Employee)">
                    @endforeach
                    @foreach($team->users->where('pivot.role', 'user') as $user)
                        <img src="{{$user->avatar()}}" alt="{{$user->name}}" class="tooltip_1 rounded-circle width-50px" title="{{$user->name}} (User)">
                    @endforeach
                </td>
                <td>
