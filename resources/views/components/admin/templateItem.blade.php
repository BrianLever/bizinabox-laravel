    ...
    <table class="table table-hover ajaxTable datatable {{$selector}}">
        ...
        @foreach($templates as $key=>$template)
            <tr>
                ...
                <td>
                    {{$template->name}}
                    <a class="text-black hover-none" href="{{route('admin.template.item.preview', $template->slug)}}" target="_blank"><i class="la la-external-link"></i></a>
                </td>
                <td>
                ...
                <td>
                    @if($template->version === 1)
                        <a href="{{route('admin.template.item.edit', $template->id)}}" class="tab-link btn btn-outline-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                    @else
                        <a href="{{route('admin.template.item.editPages', $template->id)}}" class="tab-link btn btn-info btn-sm m-1	p-2 m-btn m-btn--icon edit_btn">
                    @endif
                        <span>
