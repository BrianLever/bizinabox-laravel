module.exports = {
  init: function (MediumEditor) {
    ...
    var colorPicker = MediumEditor.extensions.form.extend({
      ...
      updateForm: function () {
        let color = window.getComputedStyle(window.getSelection().anchorNode.parentElement, null).getPropertyValue('color')
        ...
        if (this.colorItems) {
          this.colorItems.forEach((item) => {
            let ItemContent = item.getElementsByClassName('content')[0]
            let backgroundColor = window.getComputedStyle(ItemContent, null).getPropertyValue('background-color')
      ...
      createForm: function () {
        ...
        this.base.options.colors.forEach((color) => {
          let colorItemElement = doc.createElement('button')
          colorItemElement.className = 'color-item'

          let colorContentElement = doc.createElement('div')
        ...
        let themeSelector = doc.createElement('div')
