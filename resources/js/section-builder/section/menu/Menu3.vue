  ...
  <div class="bz-section-container bz-sec--sectionName-root" :class="{ [breakPoint]: true }">
    <bz-background :setting="background" :size="sectionSize">
      <bz-container>
        ...
        <bz-items :edit="edit" v-model="data.items" :cols="setting.column" :shadow="false" :spacing="false">
            ...
            <div class="bz-col-12 py-0">
              <div class="bz-row" :class="{ 'bz-row-reverse': setting.layouts.alignment === 'right' }">
                <div class="bz-col-8 py-0">
                  <bz-alignment :alignment="setting.layouts.alignment">
                    <bz-text :bold="true" :edit="edit" v-model="item.title" v-if="setting.listElements.title" :size="'1.1em'" :text-color="theme.primaryColor" />
                  </bz-alignment>
                </div>
                <div class="bz-col-4 py-0">
                  <bz-alignment :alignment="setting.layouts.alignment">
                    <bz-text :edit="edit" v-model="item.price" v-if="setting.listElements.price" :size="'1.1em'" :text-color="theme.primaryColor" />
                  </bz-alignment>
                </div>
              </div>
            </div>
...
export default {
  components: { BzItems, BzText, BzSubtitle, BzTitle, BzAlignment, BzContainer, BzBackground },
  extends: BzSection,
  name: 'Menu3'
}
</script>
