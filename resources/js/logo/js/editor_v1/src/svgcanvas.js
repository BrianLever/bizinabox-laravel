...
// customConfig - An object that contains configuration data
const canvas = ($.SvgCanvas = function (container, customConfig) {
  // Namespace constants
  ...
  var curConfig = {
    ...
    initOpacity: app.config.opacity,
    initFill: {
    ...
    initFont: app.config.font,
    baseUnit: app.config.unit,
    snappingStep: app.config.snap.grid.step,
    imgPath: app.config.path.images,
    gridSnapping: app.config.snap.grid.is_enable
  }

  // Update config with new one if given
  if (customConfig) {
    $.extend(curConfig, customConfig)
  }
  ...
  let clearSvgContentElement = (canvas.clearSvgContentElement = function () {
    ...
    $(svgcontent)
      .attr({
        ...
        x: dimensions[0],
        y: dimensions[1],
        overflow: app.config.canvas.show_outside ? 'visible' : 'hidden',
        xmlns: svgns,
        'xmlns:se': se_ns,
        'xmlns:xlink': xlinkns
      })
  ...
  $.extend(all_properties.text, {
    ...
    stroke_width: 0,
    font_size: 24,
    font_family: curConfig.initFont
  })
  ...
  var getVisibleElements = (this.getVisibleElements = function (parent, excludeIds = []) {
    if (!parent) {
    ...
    $(parent)
      ...
      .each(function (i, elem) {
        try {
          if (excludeIds.indexOf(elem.id) > -1) {
            return
          }
    ...
    return cleanItems(contentElems.reverse())
  })

  var createBackground = function (fill = 'none', width, height) {
    // Check if exist
    if (!document.getElementById('canvas_background')) {
      ...
      cur_shape = canvas.addSvgElementFromJson({
        ...
        attr: {
          x: -1,
          y: -1,
          width: width || app.config.canvas.width,
          height: height || app.config.canvas.height,
          stroke: 'none',
  ...
  const recalculateAllSelectedDimensions = (this.recalculateAllSelectedDimensions = function () {
    const text = current_resize_mode === 'none' ? 'position' : 'size'
      ...
      if (N >= 3 && tlist.getItem(N - 2).type === 3 && tlist.getItem(N - 3).type === 2 && tlist.getItem(N - 1).type === 2) {
        ...
        while (c--) {
          ...
          if (child.nodeType === 1) {
            ...
            oldStartTransform = startTransform
            const childxforms = []
          ...
          while (c--) {
            ...
            if (child.nodeType === 1) {
              ...
              if (childTlist) {
                ...
                const uses = selected.getElementsByTagNameNS(svgns, 'use')
                const href = '#' + child.id
                let u = uses.length
                while (u--) {
  ...
  ;(function () {
    ...
    var mouseDown = function (evt) {
      ...
      var pt = transformPoint(evt.pageX, evt.pageY, root_sctm),
        mouse_x = pt.x * currentZoom,
        mouse_y = pt.y * currentZoom
      evt.preventDefault()
      ...
      var x = mouse_x / currentZoom,
        y = mouse_y / currentZoom,
      ...
      switch (current_mode) {
        case 'select':
          ...
          if (mouse_target != svgroot) {
            ...
            r_start_x *= currentZoom
            r_start_y *= currentZoom
          ...
          if (selectedElements[0]) {
            let target = selectedElements[0]
            let targetBbox = target.getBoundingClientRect()
            let elements = canvas.getVisibleElements()
            let calculatedTargetBbox = {}

            let contentBbox = document.getElementById('canvas_background').getBoundingClientRect()
            let targetLeft = parseFloat(targetBbox.left / currentZoom - contentBbox.left / currentZoom)
            let targetTop = parseFloat(targetBbox.top / currentZoom - contentBbox.top / currentZoom)
            let targetRight = parseFloat(targetBbox.right / currentZoom - contentBbox.right / currentZoom)
            let targetBot = parseFloat(targetBbox.bottom / currentZoom - contentBbox.bottom / currentZoom)
            ...
            for (const [key, visible] of Object.entries(elements)) {
              let calculatedVisibleBbox = {}
              let visibleBbox = visible.getBoundingClientRect()
              let visibleLeft = parseFloat(visibleBbox.left / currentZoom - contentBbox.left / currentZoom)
              let visibleTop = parseFloat(visibleBbox.top / currentZoom - contentBbox.top / currentZoom)
              let visibleRight = parseFloat(visibleBbox.right / currentZoom - contentBbox.right / currentZoom)
              let visibleBot = parseFloat(visibleBbox.bottom / currentZoom - contentBbox.bottom / currentZoom)
          ...
          let itemsForUngroup = getVisibleElements()
          let itemsSize = getSize(itemsForUngroup)

          // Check if click by not the <g> item
          if (!itemIsGroup(mouse_target)) {
            if (itemsSize > 0) {
              for (let i = 0; i < itemsForUngroup.length; i++) {
                let item = itemsForUngroup[i]
                if (item && item.tagName === 'g' && item.childNodes.length > 0) {
        ...
        case 'zoom':
          ...
          assignAttributes(
            rubberBox,
            {
              x: real_x * currentZoom,
              y: real_x * currentZoom,
              width: 0,
              height: 0,
              display: 'inline'
            },
        ...
        case 'line':
          started = true
          stroke_w = cur_shape.stroke_width <= 1 ? 5 : cur_shape.stroke_width
          addSvgElementFromJson({
            element: 'line',
            curStyles: true,
            attr: {
      ...
      if (current_mode) {
        document.getElementById('workarea').className = current_mode == 'resize' ? evt.target.style.cursor : current_mode
      }
    }
    ...
    var mouseMove = function (evt) {
      ...
      var real_x = (x = mouse_x / currentZoom)
      var real_y = (y = mouse_y / currentZoom)
      ...
      switch (current_mode) {
        ...
        case 'resize':
          ...
          if (_.includes(['se', 'sw', 'nw', 'ne'], current_resize_mode)) {
            if (!evt.shiftKey) {
              if (sx === 1) {
                sx = sy
              } else {
        ...
        case 'zoom':
          ...
          assignAttributes(
            rubberBox,
            {
              x: Math.min(r_start_x * currentZoom, real_x),
              y: Math.min(r_start_y * currentZoom, real_y),
              width: Math.abs(real_x - r_start_x * currentZoom),
              height: Math.abs(real_y - r_start_y * currentZoom)
            },
            100
          )
          break
        ...
        case 'rotate':
          ...
          if (!evt.shiftKey) {
            var snap = app.config.snap.grid.step
    ...
    var dblClick = function (evt) {
      ...
      let parent = evtTarget.parentNode
      let mouseTarget = getMouseTarget(evt)
      let tagName = mouseTarget.tagName
  ...
  function snapToObjects(offset, axis = 'left') {
    if (offset === 0) {
      return false
    }

    let distance = app.config.snap.object.step
    let selectedItems = getSelectedElems()

    try {
      if (selectedItems.length === 1) {
        let target = selectedItems[0]
        let targetBbox = snapper[target.getAttribute('id')]
        let excludeIds = ['snap_line_x', 'snap_line_y']

        // Snap group object to workarea
        if (!itemIsGroup(target)) {
          excludeIds.push('canvas_background')
        }

        let elements = canvas.getVisibleElements(false, excludeIds)
        let indent = axis === 'left' ? 'width' : 'height'

        for (const [key, visible] of Object.entries(elements)) {
          if (target !== visible) {
            let visibleBbox = snapper[visible.getAttribute('id')]
            if (!visibleBbox) {
              continue
            }

            let targetIndent = targetBbox[indent]
            let targetLeft = targetBbox[axis] + offset
            let targetRight = targetBbox[axis] + targetIndent + offset
            let halfTargetIndent = targetIndent / 2

            let visibleIndent = visibleBbox[indent]
            let visibleLeft = visibleBbox[axis]
            let visibleRight = visibleLeft + visibleIndent

            let toLeftVisible = targetBbox[axis] - visibleLeft
            let toRightVisibleWithIndent = targetBbox[axis] - (visibleLeft + visibleIndent)
            let visibleCenter = visibleLeft + visibleIndent / 2
            let centerTarget = targetLeft + targetIndent / 2
            ...
            if (_.inRange(targetLeft, visibleLeft + distance, visibleLeft - distance)) {
              let newOffset = Math.abs(toLeftVisible)
            ...
            if (_.inRange(targetLeft, visibleRight + distance, visibleRight - distance)) {
              let newOffset = Math.abs(toRightVisibleWithIndent)
            ...
            if (_.inRange(targetRight, visibleRight + distance, visibleRight - distance)) {
              let newOffset = Math.abs(toRightVisibleWithIndent + targetIndent)
            ...
            if (_.inRange(targetRight, visibleLeft + distance, visibleLeft - distance)) {
              let newOffset = Math.abs(toLeftVisible + targetIndent)
            ...
            if (_.inRange(targetLeft, visibleCenter + distance, visibleCenter - distance)) {
              let newOffset = Math.abs(visibleCenter - targetBbox[axis])
            ...
            if (_.inRange(targetLeft + targetIndent, visibleCenter + distance, visibleCenter - distance)) {
              let newOffset = Math.abs(visibleCenter - targetBbox[axis] - targetIndent)
            ...
            if (_.inRange(centerTarget, visibleCenter + distance, visibleCenter - distance)) {
              let newOffset = Math.abs(visibleCenter - targetBbox[axis] - halfTargetIndent)
  ...
  var textActions = (canvas.textActions = (function () {
    ...
    function selectWord(evt) {
      ...
      var ept = transformPoint(evt.pageX, evt.pageY, root_sctm),
        mouse_x = ept.x * currentZoom,
        mouse_y = ept.y * currentZoom
    ...
    return {
      ...
      init: function (inputElem) {
        ...
        curtext.textContent = app.config.text.default
        ...
        textActions.toSelectMode(false)

        // Show focus element on panel
  ...
  var pathActions = (canvas.pathActions = (function () {
    ...
    return {
      mouseDown: function (evt, mouse_target, start_x, start_y) {
          ...
          if (!drawn_path) {
            ...
            if (clickOnPoint) {
              ...
              if (i <= 1 && len >= 2) {
                ...
                var grip_x = svgedit.path.first_grip ? svgedit.path.first_grip[0] / currentZoom : seglist.getItem(0).x
                var grip_y = svgedit.path.first_grip ? svgedit.path.first_grip[1] / currentZoom : seglist.getItem(0).y
                ...
                if (s_seg.pathSegType === 4) {
                  ...
                  var newseg = drawn_path.createSVGPathSegCurvetoCubicAbs(abs_x, abs_y, s_seg.x1 / currentZoom, s_seg.y1 / currentZoom, grip_x, grip_y)
                }
            ...
            // else, create a new point, update path element
              ...
              if (s_seg.pathSegType === 4) {
                ...
                var newseg = drawn_path.createSVGPathSegCurvetoCubicAbs(
                  round(x),
                  round(y),
                  s_seg.x1 / currentZoom,
                  s_seg.y1 / currentZoom,
                  s_seg.x2 / currentZoom,
                  s_seg.y2 / currentZoom
                )
              }
        ...
        if (!svgedit.path.path.dragging) {
          ...
          assignAttributes(
            rubberBox,
            {
              x: start_x * currentZoom,
              y: start_y * currentZoom,
              width: 0,
              height: 0,
              display: 'inline'
            },
      ...
      mouseMove: function (evt, mouse_x, mouse_y) {
        ...
        if (current_mode === 'path') {
          ...
          if (newPoint) {
            ...
            // Set control points
            var current_pointGrip2_x = pointGrip2.getAttribute('cx') / currentZoom || 0
            var current_pointGrip2_y = pointGrip2.getAttribute('cy') / currentZoom || 0
            ...
            var cur_x = mouse_x / currentZoom
            var cur_y = mouse_y / currentZoom
            var alt_x = is_linked ? pt_x + (pt_x - cur_x) : current_pointGrip2_x
            var alt_y = is_linked ? pt_y + (pt_y - cur_y) : current_pointGrip2_y

            pointGrip2.setAttribute('cx', alt_x * currentZoom)
            pointGrip2.setAttribute('cy', alt_y * currentZoom)
            ...
            assignAttributes(ctrlLine2, {
              x1: alt_x * currentZoom,
              y1: alt_y * currentZoom,
              x2: pt_x * currentZoom,
              y2: pt_y * currentZoom,
              display: 'inline'
            })
            ...
            if (index === 0) {
              ...
              if (last.pathSegType === 6) {
                ...
                last_x = firstCtrl[0] / currentZoom
                last_y = firstCtrl[1] / currentZoom
              }
              svgedit.path.replacePathSeg(6, index, [pt_x, pt_y, this.lastCtrlPoint[0] / currentZoom, this.lastCtrlPoint[1] / currentZoom, alt_x, alt_y], drawn_path)
            }
          } else {
            ...
            if (stretchy) {
              ...
              if (prev.pathSegType === 6) {
                var prev_x = this.lastCtrlPoint[0] / currentZoom || prev.x + (prev.x - prev.x2)
                var prev_y = this.lastCtrlPoint[1] / currentZoom || prev.y + (prev.y - prev.y2)
                svgedit.path.replacePathSeg(6, 1, [mouse_x, mouse_y, prev_x * currentZoom, prev_y * currentZoom, lastgripx, lastgripy], stretchy)
              } else if (firstCtrl) {
      ...
      mouseUp: function (evt, element, mouse_x, mouse_y) {
        ...
        if (lastpointgrip) this.lastCtrlPoint = [lastpointgrip.getAttribute('cx'), lastpointgrip.getAttribute('cy')]
  ...
  this.svgCanvasToString = function () {
    ...
    return this.svgToString(svgcontent, 0)
  }
  ...
  this.svgToString = function (elem, indent) {
    ...
    if (elem) {
      ...
      if (elem.id === 'svgcontent') {
        ...
        var i = attrs.length
        var attr_names = ['width', 'height', 'xmlns', 'id', 'overflow']
        while (i--) {
          attr = attrs.item(i)
  ...
  var uniquifyElems = (this.uniquifyElems = function (g) {
    ...
    svgedit.utilities.walkTree(g, function (n) {
      ...
      if (n.nodeType === 1) {
        ...
        const href = svgedit.utilities.getHref(n)
  ...
  this.setSvgString = function (xmlString, options) {
    try {
      // convert string into XML document
      let newDoc = svgedit.utilities.text2xml(xmlString)

      // prepare svg string
      this.prepareSvg(newDoc)

      // remove old svg document
      let nextSibling = svgcontent.nextSibling
      let oldZoom = svgroot.removeChild(svgcontent)
      ...
      let nonce = getCurrentDrawing().getNonce()
      if (nonce) {
        call('setnonce', nonce)
      } else {
        call('unsetnonce')
      ...
      createBackground('none', this.contentW, this.contentH)
      ...
      // Create snap lines
      selectorManager.initSnapElements()
    } catch (e) {
      console.log(e)
      return false
    }
  ...
  this.importSvgString = function (xmlString) {
    ...
    try {
      // Get unique ID
      const uid = svgedit.utilities.encode64(xmlString.length + xmlString).substr(0, 32)
      ...
      if (useExisting) {
        ...
        symbol = svgdoc.createElementNS('http://www.w3.org/2000/svg', 'symbol')
        const defs = findDefs()
      ...
      useEl = svgdoc.createElementNS('http://www.w3.org/2000/svg', 'use')
      useEl.id = getNextId()
      useEl.className.baseVal = 'do-not-ungroup'
      setHref(useEl, '#' + symbol.id)
      ;(current_group || getCurrentDrawing().getCurrentLayer()).append(useEl)
  ...
  const getZoom = (this.getZoom = function () {
    return currentZoom
  })
  ...
  const importImage = (this.importImage = function (e) {
    ...
    if (file.type.includes('svg')) {
      ...
      reader.onloadend = function (ev) {
        let newElement = canvas.importSvgString(ev.target.result, true)
  ...
  this.getBackgroundColor = function () {
    let background = document.getElementById('canvas_background')

    if (background) {
      return background.attributes.fill.value
    }
  ...
  this.getStrokeWidth = function (element) {
    let elem = element || selectedElements[0]
    let defaultValue = 0

    try {
      let value = elem.attributes['stroke-width'].value
      if (value === '0') {
  ...
  this.getStrokeColor = function (element) {
    ...
    try {
      let value = elem.attributes.stroke.value
  ...
  this.getOpacity = function (elem) {
    let element = elem || selectedElements[0]
    let defaultValue = 1

    try {
      let value = element.attributes.opacity.value
      if (value === '0') {
        return parseInt(value)
      }
  ...
  const getBlur = (this.getBlur = function (element) {
    let elem = element || selectedElements[0]
    let defaultValue = 0
    ...
    let filter_url = elem.getAttribute('filter')
    ...
    if (filter_url) {
      ...
      if (blur) {
        value = blur.firstElementChild.getAttribute('stdDeviation')
      }
    }
  ...
  ;(function () {
    ...
    canvas.setBlurNoUndo = function (val) {
      ...
      if (val === 0) {
        ...
        changeSelectedAttributeNoUndo('stdDeviation', val, [filter.firstElementChild])
      ...
      elems.forEach(function (elem) {
        if (elem) {
          ...
          if (val === 0) {
            ...
            batchCmd.addSubCommand(new ChangeElementCommand(filter.firstElementChild, changes))

            canvas.setBlurOffsets(filter, val)
          }

          cur_command = batchCmd
          canvas.undoMgr.beginUndoableChange('stdDeviation', [filter ? filter.firstElementChild : null])
          if (complete) {
  ...
  this.getBold = function (element) {
    // If single element
    if (element) {
      return element.getAttribute('font-weight') === 'bold'
    }

    // For all selected elements
    var selectedElems = getSelectedElems().filter(Boolean)
    var isBold = true
  ...
  this.getItalic = function (element) {
    // If single element
    if (element) {
      return element.getAttribute('font-style') === 'italic'
    }
  ...
  this.getFontFamily = function (element) {
    try {
      let elem = element || getSelectedElems()[0]

      return elem.attributes['font-family'].value
    } catch (err) {
      return null
    }
  }
  ...
  this.getFontSize = function (element) {
    let elem = element || getSelectedElems()[0]
    let defaultValue = 18

    try {
      let value = elem.attributes['font-size'].value
      if (value === '0') {
        return parseInt(value)
      }
  ...
  this.getText = function () {
    var selected = getSelectedElems()[0]
    if (selected == null) {
      return ''
    }
  ...
  this.elementsAreSame = function (elements) {
    if (!elements.length || elements[0] == null) {
      ...
      var isSameElement = function (el) {
        let selectedElements = getSelectedElems()
        if (el && selectedElements[0]) return el.nodeName === selectedElements[0].nodeName
        else return null
      }
  ...
  this.convertToPath = function (elem, getBBox) {
    ...
    if (getBBox) {
      return svgedit.utilities.getBBoxOfElementAsPath(elem, this.addSvgElementFromJson, pathActions)
    }
    ...
    return svgedit.utilities.convertToPath(elem, attrs, this.addSvgElementFromJson, pathActions, clearSelection, addToSelection, hstry, addCommandToHistory)
  }
  ...
  const changeSelectedAttributeNoUndo = function (attr, newValue, elems) {
    ...
    while (i--) {
      ...
      if (oldval !== String(newValue)) {
        ...
        if (getSelectedElems().includes(elem)) {
          setTimeout(function () {
        ...
        if (angle !== 0 && attr !== 'transform') {
          ...
          while (n--) {
            ...
            if (xform.type === 4) {
              ...
              const box = svgedit.utilities.getBBox(elem)
  ...
  this.pasteElements = function (type, x, y) {
    ...
    while (len--) {
      ...
      let newX = parseFloat(copy.getAttribute('x')) + 20
      let newY = parseFloat(copy.getAttribute('y')) + 20
      copy.setAttribute('x', newX)
      copy.setAttribute('y', newY)
  ...
  let groupSelected = (this.groupSelectedElements = function (type) {
    if (!type) type = 'g'
    var cmd_str = ''
  ...
  let groupForSelect = (this.groupForSelect = function () {
    let i = selectedElements.length
    ...
    let batchCmd = new BatchCommand('Create group')
    batchCmd.addSubCommand(new InsertElementCommand(g))
    ...
    while (i--) {
      ...
      let oldNextSibling = elem.nextSibling
      let oldParent = elem.parentNode
  ...
  let getSize = (this.getSize = function (items) {
    let count = 0
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (glist.numberOfItems) {
          ...
          if (gangle && glist.numberOfItems === 1) {
            ...
            const cbox = svgedit.utilities.getBBox(elem)
  ...
  let ungroupSelected = (this.ungroupSelectedElement = function (group) {
  ...
  this.setBackground = function (color, url) {
    // Set bg color
    let background = document.getElementById('canvas_background').setAttribute('fill', color)

    //        let bg = getElem('canvasBackground');
    //        let border = $(bg).find('rect')[0];
    //        let bg_img = getElem('background_image');
