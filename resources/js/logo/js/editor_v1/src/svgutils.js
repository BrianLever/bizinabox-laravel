...
;(function () {
  ...
  svgedit.utilities.convertToPath = function (elem, attrs, addSVGElementFromJson, pathActions, clearSelection, addToSelection, hstry, addCommandToHistory) {
  ...
  svgedit.utilities.getBBox = function (elem) {
    ...
    switch (elname) {
      ...
      default:
        ...
        } else if (visElems_arr.includes(elname)) {
          if (selected) {
            try {
              ret = selected.getBBox()
            } catch (err) {
  ...
  svgedit.utilities.getRotationAngle = function (elem, to_rad) {
    ...
    return getRotationAngleFromTransformList(tlist, to_rad)
  }
  ...
  svgedit.utilities.assignAttributes = function (elem, attrs, suspendLength, unitCheck) {
    for (const [key, value] of Object.entries(attrs)) {
      const ns = key.substr(0, 4) === 'xml:' ? XMLNS : key.substr(0, 6) === 'xlink:' ? XLINKNS : null
      ...
      if (ns) {
        ...
        svgedit.units.setUnitAttr(elem, key, value)
      }
    }
  }
