...
;(function () {
  ...
  svgedit.draw.Drawing = function (svgElem, opt_idPrefix) {
    if (!svgElem || !svgElem.tagName || !svgElem.namespaceURI || svgElem.tagName != 'svg' || svgElem.namespaceURI != svg_ns) {
    ...
    for (var i = 0; i < numchildren; ++i) {
      ...
      if (child && child.nodeType == 1) {
        if (child.tagName == 'g') {
          ...
          if (name === layerOne) {
            try {
              child.classList.add('only_svg_content')
            } catch (e) {}
          }

          // Add id for group containers
        }
