...
class DomainController extends AdminController
{
    ...
    public function check(Request $request)
    {
        try {
            ...
            if($response['status']==0)
            {
                ...
                {
                    if(current($result)['Domain']==$request->domain)
                    {
                        $views[0]= view("components.domain.record", current($result))->render()."<br><hr>";
                    }else {
                        $views[$key]= view("components.domain.record", current($result))->render();
                        $key++;
                    }
                    $domainArray[getDomainTld(current($result)['Domain'])] = current($result);
                }
    ...
    public function loadMore()
    {
        try {
            ...
            if($response['status']==0)
            {
                ...
                foreach($response['result'] as $result)
                {
                    $views[]= view("components.domain.record", current($result))->render();
    ...
    public function getNow(Request $request)
    {
        try {
            ...
            $result = current($response['result'][0]);
            ...
            $registerResult = current($resp['result'][0]);
