...
class ListController extends AdminController
{
    ...
    public function store(Request $request)
    {
        try{
            ...
            if($request->module_limit!=-1&&($request->module_limit-$module_counts)<0) return response()->json(['status'=>0, 'data'=>['Your max module limit count is '. $request->module_limit]]);
            if($request->featured_module_limit!=-1&&($request->featured_module_limit-$featured_counts)<0) return response()->json(['status'=>0, 'data'=>['Your max featured module limit count is '.$request->featured_module_limit]]);
