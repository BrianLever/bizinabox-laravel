...
class AccountController extends AdminController
{
    ...
    public function edit($id)
    {
        ...
        if($account->domain->status==0) return back()->with("error", 'Account domain status is inactive now. You can\'t edit accounts under inactive domain.');
    ...
    {
        $account = $this->model->with('domain')->findorfail($id);
        if($account->domain->status==0) return response()->json(['status'=>0, 'data'=>['Account domain status is inactive now. You can\'t edit accounts under inactive domain.']]);
