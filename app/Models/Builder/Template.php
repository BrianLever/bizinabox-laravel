...
class Template extends BaseModel implements HasMedia
{
    ...
    public function getTemplate($request, $pagination=10)
    {
        if($request->keyword=='')
        {
        ...
        $templates = $templates->orderBy('templates.created_at', 'DESC')->paginate($pagination);
        return $templates;
    }
    public function filterTemplate($request)
    {
        try {
            ...
            $templates = $this->getTemplate($request);
            $view = view('components.front.templateItem', compact('templates', 'title', 'category'))->render();
