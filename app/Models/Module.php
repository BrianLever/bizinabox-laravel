...
{
    use InteractsWithMedia, Ratable, RecurringPrice, PurchaseFollowUp, Meetable, Formable;
    ...
    public function filterItem($request)
    {
        try {
            ...
            $title = '<li class="breadcrumb-item"><a href="#/all">Modules</a></li>';
            $category = null;
            if($request->keyword=='')
            {
            ...
            $query2 = $query->where('modules.status', 1)
                ->select('modules.id', 'modules.name', 'modules.slug','modules.featured', 'modules.new','modules.description')
            ...
            $view = view('components.front.moduleItem', compact('items', 'title', 'category'))->render();
