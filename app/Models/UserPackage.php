...
class UserPackage extends BaseModel
{
    ...
    public function storeItemFromPurchase($item, $user, $orderItem)
    {
        for ($k = 0; $k < $item['quantity']; $k++)
        {
            ...
            $package->modules = json_encode($item['item']->modules->pluck("slug")->toArray() ?? []);
            $package->price = $item['parameter'];
            $package->save();

            if($package->domain>0)
            {
                Todo::storeItem($user, 'domain', 'App\Models\UserPackage', $package->id, $orderItem->id);
            }
            if($package->website>0)
            {
                Todo::storeItem($user, 'website', 'App\Models\UserPackage', $package->id, $orderItem->id, $package->website);
            }
            if ($item['item']['meeting'] == 1 && $item['item']->meetingSet()->exists()) {
                ...
                Todo::storeItem($user, 'appointment', 'App\Models\UserMeeting', $meeting->id, $orderItem->id, $userMeeting->meeting_number);
            }
            if ($item['item']['form'] == 1 && $item['item']->getForm()->exists()) {
                $form = new UserForm();
                $form->createUserForm($package, $item['item'], $user);
                Todo::storeItem($user, 'form', 'App\Models\UserForm', $form->id, $orderItem->id);
            }
