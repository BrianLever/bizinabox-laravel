...
class Package extends BaseModel implements HasMedia
{
    ...
    public function updateModuleRule($request)
    {
        ...
        if($request->featured_module_unlimit==null)
        {
            $rule['featured_module_count'] = 'required|numeric|min:0';
        }
    ...
    public function updateModule($request)
    {
        ...
        $item->featured_module = $request->featured_module_unlimit? -1:$request->featured_module_count;
    ...
    public function filterItem($request)
    {
        try {
            ...
            $view = view('components.front.packageItem', compact('items'))->render();
