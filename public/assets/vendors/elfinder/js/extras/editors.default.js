  ...
  return [
    {
      ...
      load: function (base) {
        var self = this,
          ...
          init = function (editor) {
            var $base = $(base),
              ...
              tmpDiv = [$('<div class="tui-image-editor-submenu"/>').appendTo(tmpContainer), $('<div class="tui-image-editor-controls"/>').appendTo(tmpContainer)],
            ...
            if (self.file.mime === 'image/jpeg') {
              ...
              $('<div class="ui-dialog-buttonset elfinder-edit-extras elfinder-edit-extras-quality"/>')
                .append($('<span>').html(fm.i18n('quality') + ' : '), quty, $('<span/>'))
        ...
        if (!self.confObj.editor) {
          loader = $.Deferred();
          fm.loadCss([cdns.tui + '/tui-color-picker/latest/tui-color-picker.css', cdns.tui + '/tui-image-editor/' + ver + '/tui-image-editor.css']);
          if (fm.hasRequire) {
            require.config({
              paths: {
            ...
            fm.loadScript(
              [cdns.fabric + '/fabric.min.js', cdns.tui + '/tui.code-snippet/latest/tui-code-snippet.min.js'],
              function () {
                fm.loadScript(
      ...
      load: function (base) {
        var self = this,
          ...
          init = function (onload) {
            ...
            if (!container.length) {
              container = $('<div id="elfinder-pixo-container" class="ui-front"/>')
                .css({
                  position: 'fixed',
                  top: 0,
                  right: 0,
          ...
          start = function () {
            ...
            ace.config.loadModule('ace/ext/modelist', function () {
              ...
              taBase
                ...
                .append(' (' + self.file.mime + ' : ' + mode.split(/[\/\\]/).pop() + ')');
              editor.setOptions({
                mode: 'ace/mode/' + mode
              });
              ...
              .append(
                $('<button>Ace editor setting</button>')
                  ...
                  .on('click', function () {
                    ...
                    $('#ace_settingsmenu').css('font-size', '80%').find('div[contains="setOptions"]').hide().end().parent().appendTo($('#elfinder'));
                  })
              )
      ...
      load: function (textarea) {
        ...
        if (!self.confObj.loader) {
          ...
          if (useRequire) {
            ...
            require(['codemirror', 'codemirror/addon/mode/loadmode.min', 'codemirror/mode/meta.min'], function (CodeMirror) {
            ...
            self.fm.loadScript(
              ...
              function () {
                self.fm.loadScript([cmUrl + '/addon/mode/loadmode.min.js', cmUrl + '/mode/meta.min.js'], function () {
                  self.confObj.loader.resolve(CodeMirror);
                });
              },
              { loadType: 'tag' }
      ...
      load: function (textarea) {
        var self = this,
          ...
          start = function (SimpleMDE) {
            ...
            // fit height to base
            editorBase.css('min-height', '50px').children('.CodeMirror-scroll').css('min-height', '50px');
              ...
              .then(function (editor) {
                ...
                if (editor.ui.view.toolbar && (mode === 'classic' || mode === 'decoupled-document')) {
                  $(editnode)
                    ...
                    .append($(editor.ui.view.toolbar.element).css({ marginRight: '-1em', marginLeft: '-1em' }));
                }
                if (mode === 'classic') {
    ...
    },
    {
      ...
      load: function (textarea) {
        ...
        if (!self.confObj.loader) {
          self.confObj.loader = $.Deferred();
          self.fm.loadScript(
            [fm.options.cdns.tinymce + (fm.options.cdns.tinymce.match(/\.js/) ? '' : '/tinymce.min.js')],
            function () {
              self.confObj.loader.resolve();
            },
            {
            ...
            dfdInit.done(function (data) {
              ...
              if (confObj.api) {
                ...
                dfd.done(function (d) {
                  ...
                  $.each(d, function (i, o) {
                    btns[set.useTabs ? 'children' : 'find']('.onlineconvert-category-' + o.category)
            ...
            $.each(set.conv, function (t, c) {
              ...
              $.each(c, function (n, o) {
                ...
                if (fm.uploadMimeCheck(confObj.ext2mime[ext], file.phash)) {
                  type.append(
                    $('<div class="elfinder-edit-onlineconvert-button onlineconvert-' + nl + '"/>')
