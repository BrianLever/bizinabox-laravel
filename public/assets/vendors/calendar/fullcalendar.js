...
----------------------------------------------------------------------------------------------------------------------*/
  ...
  function isRangesEqual(range0, range1) {
    return (
      ((range0.start && range1.start && range0.start.isSame(range1.start)) || (!range0.start && !range1.start)) &&
  ...
  var YearView = (FC.YearView = View.extend({
      ...
      for (n = 0; n < this.nbMonths; n++) {
        ...
        if (n % monthsPerRow === 0 && n > 0 && !hiddenMonth) {
          ...
          if (this.isBootstrap) {
            ...
            s += '<td class="fc-year-month-border fc-last"></td>' + '</tr><tr>' + '<td class="fc-year-month-border fc-first"></td>';
          }
        }
    ...
    dayBind: function (days) {
      ...
      days.click(function (ev) {
        var self = _this;
        self.trigger('dayClick', self, FC.moment(this.getAttribute('data-date')), ev);
      });
  ...
  var ListViewGrid = Grid.extend({
    ...
    fgSegHtml: function (seg) {
      ...
      return (
        ...
        classes.join(' ') +
        '">' +
        (this.displayEventTime ? '<td class="fc-list-item-time ' + view.widgetContentClass + '">' + (timeHtml || '') + '</td>' : '') +
