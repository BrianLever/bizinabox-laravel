...
(function () {
  ...
  _cb.addButton('preview', button_html, '.previewcontent-button', function (e) {
    ...
    if (!document.querySelector('.is-wrapper')) {
      ...
      // ContentBox
      var content = jQuery('.is-wrapper').data('contentbox').html();
...
function getFramedWindow(f) {
  if (f.parentNode == null) f = document.body.appendChild(f);
  var w = f.contentWindow || f.contentDocument;
  if (w && w.nodeType && w.nodeType == 9) w = w.defaultView || w.parentWindow;
  return w;
}
