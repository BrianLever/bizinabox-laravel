    ...
    if (c_tab === data.from_type && c_item == data.from_id) {
      ...
      if (data.status === 1) {
        if (obj.length === 0) {
          $('.answer-div .typing-watch').append(
            `<p class='live-chat-typing' data-id="${data.from_id}" data-type="${data.from_type}">${data.name} is typing <img class="typing_img" src="/assets/img/typing.gif"></p>`
          )
        }
      } else {
        obj.remove()
      ...
      if (data.status === 1) {
        if (selector.length === 0) {
          $(`.chat_list_item[data-id="${data.from_id}"] .last_msg`).prepend(`<p class="typing_div"><img class="typing_img" src="/assets/img/typing.gif"></p>`)
        }
      } else {
        selector.remove()
      }
...
function handleTyeping(data) {
  if (data.from_type !== 'user' || data.from_id !== my_id) {
    if (c_tab === data.to_type && c_item == data.to_id) {
      ...
      if (data.status === 1) {
        if (selector.length === 0) {
          $(`.chat_list_item[data-id="${data.to_id}"] .last_msg`).prepend(`<p class="typing_div"><img class="typing_img" src="/assets/img/typing.gif"></p>`)
        }
      } else {
        selector.remove()
      }
