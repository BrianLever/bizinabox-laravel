...
function submitNoteForm() {
  if (!$('#note').val()) {
    ...
    $.ajax({
      url: '/account/note',
      method: 'post',
      data: { note: $('#note').val(), _token: token },
      success: function (result) {
        if (result.status === 1) {
          $('#note').val(null)
          getNote()
...
$(document).on('contextmenu', '.tooltip_icon', function (e) {
  e.preventDefault()
  var title = $(this).tooltipster('content')
  console.log(title)
  $('#title_edit_modal').modal('show')
  $('#title_edit_textarea').val(title)
  $('#title_edit_page').val($(this).data('page'))
  $('#title_edit_id').val($(this).data('id'))
  tinymce.init({
    selector: '#title_edit_textarea',
    menubar: false,
    resize: 'both',
    statusbar: false,
    toolbar: ' forecolor backcolor link unlink',
